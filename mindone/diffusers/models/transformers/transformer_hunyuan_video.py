...
class HunyuanVideoAttnProcessor2_0:
    ...
    def __call__(
        ...
        if hasattr(self, "add_q_proj") and attn.add_q_proj is None and encoder_hidden_states is not None:
        ...
        _, k_len, _ = key.shape
        query = query.reshape((bs, q_len, attn.heads, -1)).transpose((0, 2, 1, 3))  # BSH -> BSND -> BNSD
        key = key.reshape((bs, k_len, attn.heads, -1)).transpose((0, 2, 1, 3))
        value = value.reshape((bs, k_len, attn.heads, -1)).transpose((0, 2, 1, 3))
        # 2. QK normalization
        ...
        if hasattr(self, "add_q_proj") and attn.add_q_proj is not None and encoder_hidden_states is not None:
            encoder_query = attn.add_q_proj(encoder_hidden_states)
            encoder_key = attn.add_k_proj(encoder_hidden_states)
            encoder_value = attn.add_v_proj(encoder_hidden_states)
            bs, q_len, _ = encoder_query.shape
            _, k_len, _ = encoder_key.shape
            encoder_query = encoder_query.reshape((bs, q_len, attn.heads, -1)).transpose((0, 2, 1, 3))
            encoder_key = encoder_key.reshape((bs, k_len, attn.heads, -1)).transpose((0, 2, 1, 3))
            encoder_value = encoder_value.reshape((bs, k_len, attn.heads, -1)).transpose((0, 2, 1, 3))
        ...
        hidden_states = mint.flatten(hidden_states.transpose((0, 2, 1, 3)), 2, 3)
...
class HunyuanVideoIndividualTokenRefiner(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            ...
            self_attn_mask_1 = attention_mask.reshape((batch_size, 1, 1, seq_len)).tile(
                (1, 1, seq_len, 1),
            )
...
class HunyuanVideoTransformerBlock(nn.Cell):
    ...
    def construct(
        ...
        temb: Tensor,
        attention_mask: Optional[Tensor] = None,
        freqs_cis: Optional[Tuple[Tensor, Tensor]] = None,
    ) -> Tuple[Tensor, Tensor]:
        # 1. Input normalization
