...
from ...utils import scale_lora_layers, unscale_lora_layers

# from ...utils.mindspore_utils import maybe_allow_in_graph
...
USE_PEFT_BACKEND = False  # FIXME: should be removed when PEFT is supported
...
def get_timestep_embedding(
    ...
    emb = timesteps[:, None].to(ms.float32) * emb[None, :]
...
class QwenTimestepProjEmbeddings(ms.nn.Cell):
...
class QwenEmbedRope(ms.nn.Cell):
...
class QwenDoubleStreamAttnProcessor2_0:
    ...
    def __call__(
        ...
        img_query = img_query.view(*img_query.shape[:-1], attn.heads, -1)
        img_key = img_key.view(*img_key.shape[:-1], attn.heads, -1)
        img_value = img_value.view(*img_value.shape[:-1], attn.heads, -1)

        txt_query = txt_query.view(*txt_query.shape[:-1], attn.heads, -1)
        txt_key = txt_key.view(*txt_key.shape[:-1], attn.heads, -1)
        txt_value = txt_value.view(*txt_value.shape[:-1], attn.heads, -1)
        ...
        # FIXME: support dispatch_attention_fn for future diffusers version
        # joint_hidden_states = dispatch_attention_fn(
        if self._attention_backend is None or self._attention_backend == "native":
            ...
            raise NotImplementedError("Flash attention is not supported for QwenDoubleStreamAttnProcessor2_0")
            # if attention_mask is not None:
            #     attention_mask = attn.prepare_attention_mask(attention_mask, sequence_length, batch_size)
            #     attention_mask = attention_mask.view(batch_size, attn.heads, -1, attention_mask.shape[-1])
            # hidden_states = self.fa(joint_query, joint_key, joint_value, None, None, None, attention_mask)[3]
...
class QwenImageTransformerBlock(ms.nn.Cell):
    def __init__(
        ...
        self.img_mod = ms.nn.SequentialCell(
            mint.nn.SiLU(),
        ...
        self.txt_mod = ms.nn.SequentialCell(
            mint.nn.SiLU(),
            mint.nn.Linear(dim, 6 * dim, bias=True),  # For scale, shift, gate for norm1 and norm2
        )
...
# FIXME: support CacheMixin for Diffusion cache
