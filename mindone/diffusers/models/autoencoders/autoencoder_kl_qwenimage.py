...
class QwenImageRMS_norm(ms.nn.Cell):
    r"""
...
class QwenImageUpsample(mint.nn.Upsample):
    r"""
    Perform upsampling while ensuring the output tensor has the same data type as the input.

    Args:
        x (mindspore.Tensor): Input tensor to be upsampled.

    Returns:
        mindspore.Tensor: Upsampled tensor with the same data type as the input.
    """
...
class QwenImageResample(ms.nn.Cell):
    r"""
    ...
    def __init__(self, dim: int, mode: str) -> None:
        ...
        # layers
        if mode == "upsample2d":
            self.resample = ms.nn.SequentialCell(
...
class QwenImageResidualBlock(ms.nn.Cell):
    r"""
    ...
    if attn_mask is not None:
        if attn_mask.dtype == ms.bool_:
            ...
            attn_mask = attn_mask.masked_fill(
                (1 - attn_mask).to(ms.bool_), ms.tensor(np.finfo(np.float16).min, dtype=ms.float16)
            )
        ...
        if is_causal:
            ...
            attn_bias = ops.masked_fill(
                attn_bias, mint.logical_not(temp_mask), ms.tensor(np.finfo(np.float16).min, dtype=ms.float16)
            )
...
class QwenImageAttentionBlock(ms.nn.Cell):
    r"""
...
class QwenImageMidBlock(ms.nn.Cell):
    """
    ...
    def __init__(self, dim: int, dropout: float = 0.0, non_linearity: str = "silu", num_layers: int = 1):
        ...
        self.attentions = ms.nn.CellList(attentions)
        self.resnets = ms.nn.CellList(resnets)
...
class QwenImageEncoder3d(ms.nn.Cell):
    r"""
    A 3D encoder module.
    ...
    def __init__(
        ...
        # downsample blocks
        self.down_blocks = ms.nn.CellList([])
...
class QwenImageUpBlock(ms.nn.Cell):
    """
    ...
    def __init__(
        ...
        if upsample_mode is not None:
            self.upsamplers = ms.nn.CellList([QwenImageResample(out_dim, mode=upsample_mode)])
    ...
    def construct(self, x, feat_cache=None, feat_idx=[0]):
        ...
        Returns:
            mindspore.Tensor: Output tensor
        """
...
class QwenImageDecoder3d(ms.nn.Cell):
    r"""
    A 3D decoder module.
...
class AutoencoderKLQwenImage(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def encode(
        ...
        Args:
            x (`mindspore.Tensor`): Input batch of images.
    ...
    def decode(self, z: ms.Tensor, return_dict: bool = True) -> Union[DecoderOutput, ms.Tensor]:
        ...
        Args:
            z (`mindspore.Tensor`): Input batch of latent vectors.
            return_dict (`bool`, *optional*, defaults to `True`):
    ...
    def tiled_encode(self, x: ms.Tensor) -> AutoencoderKLOutput:
        ...
        Args:
            x (`mindspore.Tensor`): Input batch of videos.
    ...
    def construct(
        ...
        """
        Args:
            sample (`mindspore.Tensor`): Input sample.
            return_dict (`bool`, *optional*, defaults to `True`):
