...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = QwenImageImg2ImgPipeline.from_pretrained("Qwen/Qwen-Image")
...
def retrieve_latents(encoder_output: ms.Tensor, generator: Optional[Any] = None, sample_mode: str = "sample"):
...
# Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.calculate_shift
def calculate_shift(
    image_seq_len,
...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    Returns:
        `Tuple[mindspore.Tensor, int]`: A tuple where the first element is the timestep schedule from the scheduler and the
...
class QwenImageImg2ImgPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    # Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline._extract_masked_hidden
    ...
    # Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline._get_qwen_prompt_embeds
    def _get_qwen_prompt_embeds(
        ...
        txt_tokens_np = self.tokenizer(
            txt, max_length=self.tokenizer_max_length + drop_idx, padding=True, truncation=True, return_tensors="np"
        )
        input_ids = ms.Tensor(txt_tokens_np["input_ids"])
        attention_mask = ms.Tensor(txt_tokens_np["attention_mask"])
        ...
        prompt_embeds = mint.stack(
            [
                mint.cat([u, mint.zeros((max_seq_len - u.shape[0], u.shape[1]), dtype=u.dtype)])
                for u in split_hidden_states
            ]
        )
        encoder_attention_mask = mint.stack(
            [mint.cat([u, mint.zeros((max_seq_len - u.shape[0],), dtype=u.dtype)]) for u in attn_mask_list]
        )
    ...
    def _encode_vae_image(self, image: ms.Tensor, generator: ms.Generator):
    ...
    # Copied fromCopied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline.encode_prompt
    def encode_prompt(
        self,
    ...
    @staticmethod
    # Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline._prepare_latent_image_ids
    ...
    @staticmethod
    # Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline._pack_latents
    ...
    @staticmethod
    # Copied from mindone.diffusers.pipelines.qwenimage.pipeline_qwenimage.QwenImagePipeline._unpack_latents
    ...
    def prepare_latents(
        ...
        if image.ndim == 4:
            ...
            raise ValueError(f"Expected image dims 4 or 5, got {image.ndim}.")
    ...
    def __call__(
        ...
        Args:
            ...
            negative_prompt (`str` or `List[str]`, *optional*):
                ...
                not greater than `1`).
            image (`mindspore.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[mindspore.Tensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
            ...
            generator (`ms.Generator` or `List[ms.Generator]`, *optional*):
                One or a list of generators to make generation deterministic.
            ...
            attention_kwargs (`dict`, *optional*):
                ...
                `self.processor` in
                [mindone.diffusers.models.attention_processor](https://github.com/mindspore-lab/mindone).
        ...
        latent_timestep = timesteps[:1].broadcast_to((batch_size * num_images_per_prompt,))
        ...
        txt_seq_lens = (
            mint.sum(prompt_embeds_mask, dim=1).asnumpy().tolist() if prompt_embeds_mask is not None else None
        )
        negative_txt_seq_lens = (
            mint.sum(negative_prompt_embeds_mask, dim=1).asnumpy().tolist()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                timestep = t.reshape(1).broadcast_to((latents.shape[0],)).to(latents.dtype)
