...
# XLA is not applicable in MindSpore execution
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = QwenImageInpaintPipeline.from_pretrained("Qwen/Qwen-Image")
...
class QwenImageInpaintPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def check_inputs(
        ...
        if negative_prompt_embeds is not None and negative_prompt_embeds_mask is None:
            raise ValueError(
                "If `negative_prompt_embeds` are provided, `negative_prompt_embeds_mask` also have to be passed. Make sure to\
                      generate `negative_prompt_embeds_mask` from the same text encoder that was used to generate `negative_prompt_embeds`."
            )
    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`mindspore.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[mindspore.Tensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                ...
                1)`, or `(H, W)`.
            mask_image_latent (`mindspore.Tensor`, `List[mindspore.Tensor]`):
            ...
            generator (`mindspore.Generator` or `List[mindspore.Generator]`, *optional*):
                One or a list of MindSpore generators to make generation deterministic.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # MindSpore execution does not require explicit XLA step markers
