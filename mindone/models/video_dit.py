...
class VideoDiT_Factorised_Encoder(nn.Cell):
    ...
    def construct(self, x, t, y=None, text_embed=None):
        ...
        len_spatial_blocks = len(self.blocks) // 2
        for block in self.blocks[:len_spatial_blocks]:
            x = block(x, c)  # (N, T, D)
...
def VideoDiT_XL_2(identifier="attention", **kwargs):
    if identifier == "attention":
        ...
        return VideoDiT_Factorised_Encoder(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
    elif identifier == "sd":
