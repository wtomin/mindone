...
class Gemma3nModelOutputWithPast(BaseModelOutputWithPast):
    ...
    audio_hidden_states (`mindspore.Tensor`, *optional*):
        A `mindspore.Tensor` of size `(batch_size, num_images, sequence_length, hidden_size)`.
    ...
    audio_hidden_states: Optional[ms.Tensor] = None
...
class Gemma3nCausalLMOutputWithPast(ModelOutput):
    ...
    logits (`mindspore.Tensor` of shape `(batch_size, sequence_length, config.text_config.vocab_size)`):
...
class Gemma3nRMSNorm(ms.nn.Cell):
    def __init__(self, dim: int, eps: float = 1e-6, with_scale: bool = True):
        ...
        if self.with_scale:
            ...
            self.register_buffer("weight", ms.Tensor(1.0), persistent=False)

    def _norm(self, x):
        return x / mint.sqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)
...
class Gemma3nAudioRelativePositionEmbedding(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        ...
        self.pos_proj = mint.nn.Linear(self.channels, self.num_heads * self.head_dim, bias=False)

        min_timescale = 1.0
        max_timescale = 1.0e4
        num_timescales = self.channels // 2
        log_timescale_increment = math.log(float(max_timescale) / float(min_timescale)) / max(num_timescales - 1, 1)
        inv_timescales = min_timescale * mint.exp(mint.arange(num_timescales) * -log_timescale_increment)
    ...
    def _get_timing_signal_1d_pos(self, position: ms.Tensor, dtype: ms.dtype) -> ms.Tensor:
        position = position.float().unsqueeze(-1)
        scaled_time = position * self.inv_timescales.to(dtype=ms.float32)
        timing_signal = mint.cat([mint.sin(scaled_time), mint.cos(scaled_time)], dim=-1)
        return timing_signal.type(dtype)

    def _relative_shift(
        self,
        term_bd_before_shift: ms.Tensor,
        batch_size: int,
        num_heads: int,
        ...
        term_bd_padded = mint.nn.functional.pad(term_bd_before_shift, padding_tuple)
    ...
    def construct(self, queries: ms.Tensor, keys: ms.Tensor) -> ms.Tensor:
        ...
        term_ac = mint.matmul(queries_p, keys_p_t)  # [B, N, U, W, C]

        # term_bd: Query-Position interaction
        # Original einsum: term_bd_unshifed = mint.einsum('buwnh,fnh->bnuwf', queries, sin_emb)
        ...
        # Result: [B, N, U*W, F]
        term_bd_unshifed_matmul = mint.matmul(q_reshaped, s_permuted)
...
class Gemma3nAudioAttention(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.per_dim_scale = ms.Parameter(mint.zeros((self.head_dim,)))

        self.q_proj = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        self.k_proj = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        self.v_proj = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)

        q_scale = self.head_dim**-0.5
        r_softplus_0 = 1.0 / mint.nn.functional.softplus(ms.Tensor(0.0))
        self.register_buffer("q_scale", (q_scale * r_softplus_0).clone().detach(), persistent=False)

        lower_causal_mask = mint.tril(
            mint.ones((self.context_size, self.chunk_size), dtype=ms.bool_),
            diagonal=0,
        ).T
        upper_causal_mask = mint.tril(
            mint.ones((self.chunk_size, self.context_size), dtype=ms.bool_),
            diagonal=self.max_past_horizon + self.max_future_horizon,
        )
        local_causal_valid_mask = mint.ones((self.chunk_size, self.context_size), dtype=ms.bool_)
        ...
        self.register_buffer(
            "softcap",
            ms.Tensor(self.attention_logits_soft_cap).float(),
            persistent=False,
        )

    def _pad_dim1(self, x: ms.Tensor, pad_left: int, pad_right: int) -> ms.Tensor:
        batch, _, *tail_shape = x.shape
        left = x.new_zeros((batch, pad_left, *tail_shape))
        right = x.new_zeros((batch, pad_right, *tail_shape))
        x = mint.cat([left, x, right], dim=1)
        return x

    def _convert_to_block(self, hidden_states: ms.Tensor) -> ms.Tensor:
    ...
    def _extract_block_context(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        if hidden_states.ndim > 2 and x_unfolded.ndim > 3:  # Check if inner dimensions (like N, H) exist
            ...
            x_unfolded = ms.ops.movedim(x_unfolded, source=-1, destination=2)

        return x_unfolded.contiguous()

    def construct(self, hidden_states: ms.Tensor, mask: ms.Tensor) -> ms.Tensor:
        ...
        per_dim_scale_sp = mint.nn.functional.softplus(self.per_dim_scale)
        ...
        final_condition_for_where = mint.logical_and(
        ...
        logits = mint.where(final_condition_for_where, logits, _DTYPE_2_MIN[logits.dtype])
        probabilities = mint.nn.functional.softmax(logits, dim=-1, dtype=ms.float32).to(dtype=value_blocks.dtype)
        ...
        result_bmm = mint.bmm(prob_bun, v_bun)
...
class Gemma3nAudioCumulativeGroupNorm(ms.nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        mask_calc = mint.ones_like(x_calc, dtype=calc_dtype)

        # Cumulative Statistics Calculation
        # 1. Sum of values over reduction axes at each time step.
        sum_values_at_t = mint.sum(x_calc, dim=self.reduction_axes, keepdim=True)
        # 2. Cumulative sum of values over time.
        cum_sum_values = mint.cumsum(sum_values_at_t, dim=1)
        ...
        elements_in_group_at_t = mint.sum(mask_calc, dim=self.reduction_axes, keepdim=True)
        # 4. Cumulative count of valid elements over time.
        cum_count_elements = mint.cumsum(elements_in_group_at_t, dim=1)
        # Avoid division by zero if all preceding elements were masked.
        safe_cum_count_elements = mint.clamp(cum_count_elements, min=1.0)
        ...
        sum_sq_diff_at_t = mint.sum(squared_diff_from_mean, dim=self.reduction_axes, keepdim=True)

        # 7. Cumulative sum of squared differences over time.
        cum_sum_sq_diff = mint.cumsum(sum_sq_diff_at_t, dim=1)
        ...
        normalized_x = (x_calc - cum_mean) * mint.rsqrt(cum_variance + self.eps)
...
class Gemma3nAudioSSCPConvBlock(ms.nn.Cell):
    ...
    def construct(self, audio_encodings: ms.Tensor) -> ms.Tensor:
        ...
        audio_encodings_padded = mint.nn.functional.pad(
...
class Gemma3nAudioSubSampleConvProjection(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.input_proj_linear = mint.nn.Linear(self.input_proj_in_features, self.config.hidden_size, bias=False)
...
class Gemma3nAudioConformerAttention(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config
        self.post_in_features = self.config.hidden_size
        self.register_buffer("gradient_clipping", ms.Tensor(self.config.gradient_clipping), persistent=False)
        self.pre_attn_norm = Gemma3nRMSNorm(self.config.hidden_size)
        self.attn = Gemma3nAudioAttention(config)
        self.post = mint.nn.Linear(self.post_in_features, self.config.hidden_size, bias=False)
        self.post_norm = Gemma3nRMSNorm(self.config.hidden_size)

    def construct(self, audio_encodings: ms.Tensor, audio_mel_mask: ms.Tensor) -> ms.Tensor:
        audio_encodings_input_to_attn = audio_encodings
        audio_encodings = mint.clamp(audio_encodings, -self.gradient_clipping, self.gradient_clipping)
...
class Gemma3nAudioConformerFeedForward(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config
        ...
        self.ffw_layer_1 = mint.nn.Linear(self.config.hidden_size, self.config.hidden_size * 4, bias=False)
        self.ffw_layer_2 = mint.nn.Linear(self.config.hidden_size * 4, self.config.hidden_size, bias=False)
        self.post_layer_norm = Gemma3nRMSNorm(self.config.hidden_size)
        self.post_layer_scale = ms.Tensor(self.config.conf_residual_weight)

    def construct(self, audio_encodings: ms.Tensor) -> ms.Tensor:
        residual = audio_encodings
        audio_encodings = mint.clamp(audio_encodings, -self.gradient_clipping, self.gradient_clipping)
        audio_encodings = self.pre_layer_norm(audio_encodings)
        audio_encodings: ms.Tensor = self.ffw_layer_1(audio_encodings)
        audio_encodings = mint.nn.functional.silu(audio_encodings)
        audio_encodings: ms.Tensor = self.ffw_layer_2(audio_encodings)
        audio_encodings = mint.clamp(audio_encodings, -self.gradient_clipping, self.gradient_clipping)
        audio_encodings = self.post_layer_norm(audio_encodings)
        return residual + (audio_encodings * self.post_layer_scale)


class Gemma3nAudioConformerLightConv1d(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config

        self.pre_layer_norm = Gemma3nRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)
        self.linear_start = mint.nn.Linear(self.config.hidden_size, self.config.hidden_size * 2, bias=False)
        self.depthwise_conv1d = mint.nn.Conv1d(
        ...
        self.linear_end = mint.nn.Linear(self.config.hidden_size, self.config.hidden_size, bias=False)
    ...
    def construct(self, audio_encodings: ms.Tensor) -> ms.Tensor:
        ...
        audio_encodings = mint.nn.functional.glu(audio_encodings, dim=-1)
        # Permute for Conv1d: [B, T, D] -> [B, D, T]
        audio_encodings_permuted = audio_encodings.permute(0, 2, 1)
        # Apply manual causal padding
        audio_encodings_permuted_padded = mint.nn.functional.pad(audio_encodings_permuted, (self.causal_padding, 0))
...
class Gemma3nAudioConformerBlock(ms.nn.Cell):
    def __init__(self, config: Gemma3nAudioConfig):
        super().__init__()
        self.config = config
...
class Gemma3nAudioEncoder(PreTrainedModel):
    ...
    def __init__(self, config: Gemma3nAudioConfig):
        ...
        self.conformer = ms.nn.CellList(
            [Gemma3nAudioConformerBlock(config) for _ in range(config.conf_num_hidden_layers)]
        )

    def construct(self, audio_mel: ms.Tensor, audio_mel_mask: ms.Tensor) -> tuple[ms.Tensor, ms.Tensor]:
        """Encodes a batch of MELs.

        Args:
            audio_mel: a mindspore.Tensor of shape [batch, num_frames, num_channels,
              mel_bins].

        Returns:
            audio_encodings: a mindspore.Tensor of shape
                `[batch_size, self.config.audio_soft_tokens_per_image,
                self.config.audio_config.hidden_size]`
            audio_mel_mask: a mindspore.Tensor of shape [batch, num_frames].
        """
        ...
        indices = mint.clamp(indices, max=audio_mel_mask.shape[1] - 1)  # Ensure indices are valid

        # Expand indices for batch compatibility if B > 1 and indices is 1D.
        if audio_mel_mask.ndim > 1 and indices.ndim == 1:
            indices = indices.unsqueeze(0)
            indices = indices.broadcast_to((audio_mel_mask.shape[0], indices.shape[1]))  # [B, T_sub]
        elif (
        ...
        current_mask = mint.gather(audio_mel_mask, 1, indices)  # [B, T_sub]
...
class Gemma3nTextScaledWordEmbedding(mint.nn.Embedding):
    """
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx: int, embed_scale: float = 1.0):
        ...
        self.register_buffer("embed_scale", ms.Tensor(embed_scale), persistent=False)

    def construct(self, input_ids: ms.Tensor):
        return super().construct(input_ids) * self.embed_scale.to(self.weight.dtype)


class Gemma3nTextLaurelBlock(ms.nn.Cell):
    ...
    def __init__(self, config: Gemma3nTextConfig):
        ...
        self.linear_left = mint.nn.Linear(self.config.hidden_size, self.config.laurel_rank, bias=False)
        self.linear_right = mint.nn.Linear(self.config.laurel_rank, self.config.hidden_size, bias=False)
        self.post_laurel_norm = Gemma3nRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)

    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        laurel_hidden_states: ms.Tensor = self.linear_left(hidden_states)
        laurel_hidden_states: ms.Tensor = self.linear_right(laurel_hidden_states)
        normed_laurel_hidden_states = self.post_laurel_norm(laurel_hidden_states)
        return hidden_states + normed_laurel_hidden_states


class Gemma3nTextMLP(ms.nn.Cell):
    ...
    def _gaussian_topk(self, inputs: ms.Tensor) -> ms.Tensor:
        target_sparsity_tensor = ms.Tensor(
            self.activation_sparsity,
            dtype=ms.float32,
        )
        # Compute Normal(0,1).ppf(p) via erfinv: ppf(p) = mean + stddev * sqrt(2) * erfinv(2p-1)
        std_multiplier = mint.erfinv(target_sparsity_tensor * 2.0 - 1.0) * math.sqrt(2.0)
        std_multiplier = std_multiplier.to(inputs.dtype)
        inputs_mean = mint.mean(inputs, dim=-1, keepdim=True)
        inputs_std = mint.std(inputs, dim=-1, keepdim=True, unbiased=False)
        cutoff_x = inputs_mean + inputs_std * std_multiplier
        return mint.nn.functional.relu(inputs - cutoff_x)


class Gemma3nTextAltUp(ms.nn.Cell):
    ...
    def __init__(self, config: Gemma3nTextConfig):
        super().__init__()
        self.config = config
        self.correct_output_scale = ms.Parameter(mint.zeros(self.config.hidden_size))
        self.correction_coefs = mint.nn.Linear(self.config.altup_num_inputs, self.config.altup_num_inputs, bias=False)
        self.prediction_coefs = mint.nn.Linear(
            self.config.altup_num_inputs, self.config.altup_num_inputs**2, bias=False
        )
        self.modality_router = mint.nn.Linear(self.config.hidden_size, self.config.altup_num_inputs, bias=False)
    ...
    def predict(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        predictions = mint.matmul(hidden_states.permute(1, 2, 3, 0), all_coefs)
    ...
    def correct(self, predictions: ms.Tensor, activated: ms.Tensor) -> ms.Tensor:
        ...
        all_coefs: ms.Tensor = self.correction_coefs(modalities) + 1.0
        all_coefs = all_coefs.permute(2, 0, 1).unsqueeze(-1)

        corrected = mint.mul(innovation, all_coefs)
        corrected += predictions  # add the original input
        return corrected.contiguous().type_as(activated)

    def construct(self, corrected: ms.Tensor) -> ms.Tensor:
        """
...
class Gemma3nTextRotaryEmbedding(ms.nn.Cell):
    ...
    def construct(self, x, position_ids):
        inv_freq_expanded = self.inv_freq[None, :, None].astype(ms.float32)
        inv_freq_expanded = inv_freq_expanded.broadcast_to((position_ids.shape[0], -1, 1))
...
def repeat_kv(hidden_states: ms.Tensor, n_rep: int) -> ms.Tensor:
    """
    This is the equivalent of repeating along the head dimension (dim=1). The hidden states go from (batch,
...
def apply_rotary_pos_emb(
    ...
    Args:
        x (`mindspore.Tensor`): The tensor to embed.
...
class Gemma3nTextAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.is_kv_shared_layer and self.kv_shared_layer_index is not None and past_key_value is not None:
            ...
            # Device of past layer may be different from current one
            key_states = layer.keys[:, :, indices]
            value_states = layer.values[:, :, indices]
        else:
...
class Gemma3nTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Gemma3nTextConfig, layer_idx: int):
        ...
        self.per_layer_input_gate = mint.nn.Linear(self.hidden_size, self.hidden_size_per_layer_input, bias=False)
        self.per_layer_projection = mint.nn.Linear(self.hidden_size_per_layer_input, self.hidden_size, bias=False)
    ...
    def construct(
        ...
        **kwargs,
        ...
        first_prediction = ms.ops.multiply(first_prediction, per_layer_input)
...
class Gemma3nTextModel(Gemma3nPreTrainedModel):
    ...
    def __init__(self, config: Gemma3nTextConfig):
        ...
        self.per_layer_model_projection = mint.nn.Linear(
            self.hidden_size,
            config.num_hidden_layers * config.hidden_size_per_layer_input,
            bias=False,
        )
        ...
        self.altup_projections = ms.nn.CellList(
            [
                mint.nn.Linear(self.hidden_size, self.hidden_size, bias=False)
                for _ in range(1, self.config.altup_num_inputs)
            ]
        )

        self.altup_unembed_projections = ms.nn.CellList(
            [
                mint.nn.Linear(self.hidden_size, self.hidden_size, bias=False)
                for _ in range(1, self.config.altup_num_inputs)
            ]
        ...
        self.register_buffer("per_layer_projection_scale", ms.Tensor(self.hidden_size**-0.5), persistent=False)
        self.register_buffer("per_layer_input_scale", mint.rsqrt(ms.Tensor(2.0)), persistent=False)
    ...
    def construct(
        ...
        per_layer_inputs: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        per_layer_inputs (mindspore.Tensor, *optional*, defaults to None):
        ...
        target_magnitude = mint.mean(hidden_states_0**2, dim=-1, keepdim=True) ** 0.5
        epsilon_tensor = ms.Tensor(1e-5)

        temp_hidden_states = [hidden_states_0]
        for i in range(1, self.config.altup_num_inputs):
            # altup_proj adapted from jax.numpy.einsum("btp,pd->btd", ...)
            altup_proj = self.altup_projections[i - 1](hidden_states_0)
            current_hidden_state = altup_proj.to(
        ...
        target_magnitude = mint.mean(hidden_states[0] ** 2, dim=-1, keepdim=True) ** 0.5
        temp_hidden_states = [hidden_states[0]]
        for i in range(1, self.config.altup_num_inputs):
            # altup_unembed_projections adapted from jax.numpy.einsum("btp,pd->btd", ...)
            altup_unemb_proj: ms.Tensor = self.altup_unembed_projections[i - 1](hidden_states[i])
            current_hidden_state = altup_unemb_proj.to(
                dtype=hidden_states_0.dtype,
            )
            new_magnitude = mint.mean(current_hidden_state**2, dim=-1, keepdim=True)
            new_magnitude = mint.sqrt(mint.maximum(new_magnitude, epsilon_tensor))
    ...
    def get_per_layer_inputs(self, input_ids: ms.Tensor) -> ms.Tensor:
    ...
    def project_per_layer_inputs(
        ...
        per_layer_projection: ms.Tensor = self.per_layer_model_projection(inputs_embeds)
        per_layer_projection *= self.per_layer_projection_scale.to(dtype=inputs_embeds.dtype)
        ...
        return (per_layer_projection + per_layer_inputs) * self.per_layer_input_scale.to(dtype=inputs_embeds.dtype)
...
class Gemma3nForCausalLM(Gemma3nPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers.models.gemma3n import Gemma3nForCausalLM
...
class Gemma3nMultimodalEmbedder(ms.nn.Cell):
    ...
    def __init__(
        ...
        self.embedding = mint.nn.Embedding(self.vocab_size, self.multimodal_hidden_size)
        ...
        self.embedding_projection = mint.nn.Linear(self.multimodal_hidden_size, self.text_hidden_size, bias=False)
    ...
    def construct(
        ...
        Args:
            input_ids: A mindspore.Tensor containing the token ids to embed. Values should be in the range
                `[vocab_offset, vocab_offset + vocab_size)`.
            inputs_embeds: A mindspore.Tensor containing the soft tokens to embed.

        Returns:
            A mindspore.Tensor of embeddings with  shape `[batch_size, seq_len, self.config.text_config.hidden_size]`.
        """
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
...
class Gemma3nModel(Gemma3nPreTrainedModel):
    ...
    def get_placeholder_mask(
        ...
        special_audio_mask = special_audio_mask.unsqueeze(-1).broadcast_to(inputs_embeds.shape)
    ...
    def construct(
        self,
        input_ids: Optional[ms.Tensor] = None,  # text inputs
        pixel_values: Optional[ms.Tensor] = None,  # vision inputs
        input_features: Optional[ms.Tensor] = None,  # audio inputs
        attention_mask: Optional[ms.Tensor] = None,
        input_features_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        past_key_values: Optional[Union[list[ms.Tensor], Cache]] = None,
        ...
        **lm_kwargs,
        ...
        >>> from mindone.transformers.models.gemma3n import Gemma3nForConditionalGeneration
        ...
        if input_ids is not None:
            ...
            per_layer_inputs_mask = mint.logical_and(input_ids >= 0, input_ids < self.vocab_size_per_layer_input)
            per_layer_inputs_tokens = mint.where(per_layer_inputs_mask, input_ids, mint.zeros_like(input_ids))
            ...
            vision_input_ids = mint.where(vision_mask, input_ids, dummy_vision_token_id)
            vision_embeds = self.embed_vision(input_ids=vision_input_ids)
            expanded_vision_mask = vision_mask.unsqueeze(-1).broadcast_to(inputs_embeds.shape)
            inputs_embeds = mint.where(expanded_vision_mask, vision_embeds, inputs_embeds)
            ...
            audio_input_ids = mint.where(audio_mask, input_ids, dummy_audio_token_id)
            audio_embeds = self.embed_audio(input_ids=audio_input_ids)
            expanded_audio_mask = audio_mask.unsqueeze(-1).broadcast_to(inputs_embeds.shape)
            inputs_embeds = mint.where(expanded_audio_mask, audio_embeds, inputs_embeds)
        else:
        ...
        if input_features is not None and input_features_mask is not None:
            ...
            audio_padding_toks = ms.Tensor(
                [[self.vocab_size - 1]],
                dtype=ms.int64,
            )
            audio_padding_embs = self.embed_audio(input_ids=audio_padding_toks)
            audio_features = mint.where(audio_mask.unsqueeze(-1), audio_padding_embs, audio_features)

            audio_batch_size, audio_seq_len, audio_embed_dim = audio_features.shape
            extra_padding_tokens = self.config.audio_soft_tokens_per_image - audio_seq_len
            extra_padding_features = audio_padding_embs.broadcast_to(
                (audio_batch_size, extra_padding_tokens, audio_embed_dim)
            )

            audio_features = mint.cat((audio_features, extra_padding_features), dim=1)
    ...
    def get_audio_features(
        self, input_features: ms.Tensor, input_features_mask: ms.Tensor
    ) -> tuple[ms.Tensor, ms.Tensor]:
        """
        Projects the last hidden state from the audio encoder into language model space.

        Args:
            input_features (`mindspore.Tensor]` of shape `(num_images, seq_length, num_features)`):
               The tensors corresponding to the input audio.
            input_features_mask (`mindspore.Tensor]` of shape `(num_images, seq_length)`):
               The attention mask for the input audio.

        Returns:
            audio_features (`mindspore.Tensor`): Audio feature tensor of shape `(num_images, audio_length, embed_dim)`).
        """
...
class Gemma3nForConditionalGeneration(Gemma3nPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **lm_kwargs,
        ...
        r"""
        input_features_mask (mindspore.Tensor, *optional*, defaults to None):
            The attention mask for the input audio.
        labels (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
