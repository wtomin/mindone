...
class SmolLM3Attention(ms.nn.Cell):
...
class SmolLM3RMSNorm(ms.nn.Cell):
...
class SmolLM3RotaryEmbedding(ms.nn.Cell):
    ...
    def construct(self, x, position_ids):
        inv_freq_base = self.inv_freq[None, :, None].astype(ms.float32)
        inv_freq_expanded = inv_freq_base.broadcast_to(
            (position_ids.shape[0], self.inv_freq.shape[0], 1)
        )
        position_ids_expanded = position_ids[:, None, :].astype(ms.float32)

        freqs = mint.matmul(inv_freq_expanded, position_ids_expanded).transpose(1, 2)
...
class SmolLM3ForCausalLM(SmolLM3PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> import mindspore as ms
        >>> from mindone.transformers import AutoTokenizer, SmolLM3ForCausalLM
