...
def rotate_half(x):
    ...
    x1 = x[..., 0::2]
    x2 = x[..., 1::2]
    return mint.stack((-x2, x1), dim=-1).reshape(x.shape[:-2] + (-1,))
...
class HeliumForCausalLM(HeliumPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> inputs = tokenizer(prompt, return_tensors="np") # Changed to np for MindSpore
        >>> input_ids_ms = ms.Tensor(inputs.input_ids) # Convert to MindSpore Tensor

        >>> # Generate
        >>> generate_ids = model.generate(input_ids_ms, max_length=30).asnumpy()
        >>> tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0] # Convert back to numpy for tokenizer
