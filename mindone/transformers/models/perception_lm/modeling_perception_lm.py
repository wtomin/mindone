...
class PerceptionLMAdaptiveAvgPooling(ms.nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        hidden_states = mint.nn.functional.adaptive_avg_pool2d(hidden_states, shape)
        hidden_states = hidden_states.flatten(2).transpose(1, 2)

        return hidden_states


class PerceptionLMMultiModalProjector(ms.nn.Cell):
...
class PerceptionLMModelOutputWithPast(BaseModelOutputWithPast):
    ...
    video_hidden_states (`mindspore.Tensor`, *optional*):
        A `mindspore.Tensor` of size `(batch_size, num_videos, sequence_length, hidden_size)`.
...
class PerceptionLMModel(PerceptionLMPreTrainedModel):
    ...
    def get_image_features(
        ...
        **kwargs,
        ...
        Args:
            pixel_values (`mindspore.Tensor]` of shape `(batch_size, num_tiles, channels, height, width)`)
               The tensors corresponding to the input images.
        Returns:
            image_features (`mindspore.Tensor`): Image feature tensor of shape `(num_tiles, num_patches, embed_dim)`).
        """
    ...
    def get_placeholder_mask(
        ...
        if image_features is not None and inputs_embeds[special_image_mask].numel() != image_features.numel():
            raise ValueError(
                f"Image features and image tokens do not match: tokens: {n_image_tokens}, features {image_features.shape[:-1].numel()}"
            )

        n_video_tokens = special_video_mask.sum()
        special_video_mask = special_video_mask.unsqueeze(-1).broadcast_to(inputs_embeds.shape)
        if video_features is not None and inputs_embeds[special_video_mask].numel() != video_features.numel():
            raise ValueError(
                f"Videos features and image tokens do not match: tokens: {n_video_tokens}, features {video_features.shape[:-1].numel()}"
            )
    ...
    def construct(
        ...
        **lm_kwargs,
        ...
        if pixel_values_videos is not None:
            ...
            video_features = video_features.to(dtype=inputs_embeds.dtype)
...
class PerceptionLMForConditionalGeneration(PerceptionLMPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **lm_kwargs,
        ...
        >>> from mindone.transformers.models.perception_lm import PerceptionLMForConditionalGeneration
