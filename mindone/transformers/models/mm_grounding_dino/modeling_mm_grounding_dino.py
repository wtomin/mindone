...
from mindone.transformers.mindspore_adapter.utils import _DTYPE_2_MIN, _DTYPE_2_MAX
...
class MMGroundingDinoContrastiveEmbedding(ms.nn.Cell):
    ...
    def construct(
        ...
        if pad_len > 0:
            pad_tensor = mint.full((*res.shape[:-1], pad_len), float("-inf"), dtype=res.dtype)
            res = mint.cat([res, pad_tensor], dim=-1)
        return res
...
class MultiScaleDeformableAttention(ms.nn.Cell):
    def construct(
        ...
        for level_id, (height, width) in enumerate(value_spatial_shapes_list):
            ...
            # -> batch_size*num_heads, num_queries, num_points, 2
            sampling_grid_l_ = sampling_grids[:, :, :, level_id].transpose(1, 2)
            sampling_grid_l_ = sampling_grid_l_.reshape(batch_size * num_heads, num_queries, num_points, 2)
...
class MMGroundingDinoBiMultiHeadAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        Args:
            vision_features (`mindspore.Tensor` of shape `(batch_size, vision_sequence_length, hidden_dim)`):
                Projected flattened image features generated by the vision backbone.
            text_features (`mindspore.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`):
                Projected text features generated by the text encoder.
            vision_attention_mask (`mindspore.Tensor`, **optional**):
                Attention mask for image-to-text cross-attention. False for real tokens and True for padding tokens.
            text_attention_mask (`mindspore.Tensor`, **optional**):
                Attention mask for text-to-image cross-attention. False for real tokens and True for padding tokens.

        Returns:
            `tuple(tuple(mindspore.Tensor), tuple(mindspore.Tensor))` where each inner tuple comprises an attention
            output and weights:
            - **vision_attn_output** (`mindspore.Tensor` of shape `(batch_size, vision_sequence_length, hidden_din)`)
              --
                Output of the image-to-text cross-attention layer.
            - **vision_attn_weights** (`mindspore.Tensor` of shape `(batch_size, num_heads, vision_sequence_length,
              vision_sequence_length)`) --
                Attention weights of the image-to-text cross-attention layer.
            - **text_attn_output** (`mindspore.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`) --
                Output of the text-to-image cross-attention layer.
            - **text_attn_weights** (`mindspore.Tensor` of shape `(batch_size, num_heads, text_sequence_length,
...
class MMGroundingDinoFusionLayer(ms.nn.Cell):
    ...
    def construct(
        ...
        Args:
            ...
            text_features (`mindspore.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`):
                Projected text features generated by the text encoder.
            attention_mask_vision (`mindspore.Tensor`, **optional**):
                Attention mask for image-to-text cross-attention. False for real tokens and True for padding tokens.
            attention_mask_text (`mindspore.Tensor`, **optional**):
                Attention mask for text-to-image cross-attention. False for real tokens and True for padding tokens.

        Returns:
            `tuple(tuple(mindspore.Tensor), tuple(mindspore.Tensor))` where each inner tuple comprises an enhanced
            feature and attention output and weights:
            - **vision_features** (`mindspore.Tensor` of shape `(batch_size, vision_sequence_length, vision_dim)`) --
                Updated vision features with attention output from image-to-text cross-attention layer.
            - **vision_attn_weights** (`mindspore.Tensor` of shape `(batch_size, num_heads, vision_sequence_length,
              vision_sequence_length)`) --
                Attention weights of the image-to-text cross-attention layer.
            - **text_features** (`mindspore.Tensor` of shape `(batch_size, text_sequence_length, text_dim)`) --
...
def get_sine_pos_embed(
    ...
    Returns:
        position_embeddings (mindspore.Tensor): shape: [..., n * hidden_size].
    """
    scale = 2 * math.pi
...
class MMGroundingDinoPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        def set_param_constant(param: ms.Parameter, value: float):
            param.set_data(initializer(Constant(value), param.shape, param.dtype))

        def set_param_normal(param: ms.Parameter, mean: float = 0.0, std_val: float = 1.0):
            param.set_data(initializer(Normal(std_val, mean), param.shape, param.dtype))

        def set_param_xavier_uniform(param: ms.Parameter):
        ...
        if isinstance(module, MMGroundingDinoLearnedPositionEmbedding):
            set_param_uniform = lambda p: p.set_data(initializer(Uniform(1.0), p.shape, p.dtype))
            set_param_uniform(module.row_embeddings.weight)
            set_param_uniform(module.column_embeddings.weight)
        elif isinstance(module, MMGroundingDinoMultiscaleDeformableAttention):
            # sampling_offsets weight to zeros
            set_param_constant(module.sampling_offsets.weight, 0.0)
            # initialize sampling_offsets bias with scaled grid as in original implementation
            thetas = mint.arange(module.n_heads, dtype=ms.float32) * (2.0 * math.pi / module.n_heads)
            ...
            set_param_constant(module.attention_weights.weight, 0.0)
            set_param_constant(module.attention_weights.bias, 0.0)
            set_param_xavier_uniform(module.value_proj.weight)
            set_param_constant(module.value_proj.bias, 0.0)
            set_param_xavier_uniform(module.output_proj.weight)
            set_param_constant(module.output_proj.bias, 0.0)
        elif isinstance(module, MMGroundingDinoBiMultiHeadAttention):
            set_param_xavier_uniform(module.vision_proj.weight)
            set_param_constant(module.vision_proj.bias, 0.0)
            set_param_xavier_uniform(module.text_proj.weight)
            set_param_constant(module.text_proj.bias, 0.0)
            set_param_xavier_uniform(module.values_vision_proj.weight)
            set_param_constant(module.values_vision_proj.bias, 0.0)
            set_param_xavier_uniform(module.values_text_proj.weight)
            set_param_constant(module.values_text_proj.bias, 0.0)
            set_param_xavier_uniform(module.out_vision_proj.weight)
            set_param_constant(module.out_vision_proj.bias, 0.0)
            set_param_xavier_uniform(module.out_text_proj.weight)
            set_param_constant(module.out_text_proj.bias, 0.0)
        elif isinstance(module, MMGroundingDinoFusionLayer):
            module.vision_param.set_data(initializer(Constant(1e-4), module.vision_param.shape, module.vision_param.dtype))
            ...
            set_param_normal(module.weight, mean=0.0, std_val=std)
            if getattr(module, "bias", None) is not None:
                set_param_constant(module.bias, 0.0)
        elif isinstance(module, (mint.nn.LayerNorm, mint.nn.GroupNorm)):
            set_param_constant(module.weight, 1.0)
            set_param_constant(module.bias, 0.0)
        elif isinstance(module, mint.nn.Embedding):
            set_param_normal(module.weight, mean=0.0, std_val=std)
            if module.padding_idx is not None:
                weight_data = module.weight.asnumpy()
                weight_data[module.padding_idx] = 0
                module.weight.set_data(ms.Tensor(weight_data, dtype=module.weight.dtype))
        elif isinstance(module, MMGroundingDinoMLPPredictionHead):
            last_linear = module.layers[-1]
            set_param_constant(last_linear.weight, 0.0)
            if getattr(last_linear, "bias", None) is not None:
                set_param_constant(last_linear.bias, 0.0)

        if hasattr(module, "reference_points") and not self.config.two_stage:
            # reference_points is an Embedding
            set_param_xavier_uniform(module.reference_points.weight)
        if hasattr(module, "level_embed"):
            module.level_embed.set_data(initializer(Normal(std, 0.0), module.level_embed.shape, module.level_embed.dtype))
        if isinstance(module, MMGroundingDinoContrastiveEmbedding):
            module.bias.set_data(ms.Tensor(-math.log((1 - 0.01) / 0.01), dtype=ms.float32))
...
class MMGroundingDinoEncoderOutput(ModelOutput):
    r"""
    last_hidden_state_vision (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        Sequence of hidden-states at the output of the last layer of the vision encoder.
    last_hidden_state_text (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
    ...
    vision_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
        Tuple of `mindspore.Tensor` (one for the output of the vision embeddings + one for the output of each
        layer) of shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the vision encoder at the
        output of each layer plus the initial embedding outputs.
    text_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
        Tuple of `mindspore.Tensor` (one for the output of the text embeddings + one for the output of each layer)
...
class MMGroundingDinoTextEnhancerLayer(ms.nn.Cell):
    ...
    def construct(
        ...
        Args:
            ...
            attention_masks (`mindspore.Tensor`, *optional*):
        ...
        Returns:
            `tuple(mindspore.Tensor)` comprising two elements:
            - **hidden_states** (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`) --
                Output of the text self-attention layer.
            - **attention_weights** (`mindspore.Tensor` of shape `(batch_size, num_heads, sequence_length,
              sequence_length)`) --
...
class MMGroundingDinoEncoder(MMGroundingDinoPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        Args:
            vision_features (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Flattened feature map (output of the backbone + projection layer) that is passed to the encoder.
            vision_attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            vision_position_embedding (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
            ...
            text_features (`mindspore.Tensor` of shape `(batch_size, text_seq_len, hidden_size)`):
                Flattened text features that are passed to the encoder.
            text_attention_mask (`mindspore.Tensor` of shape `(batch_size, text_seq_len)`, *optional*):
                ...
                [What are attention masks?](../glossary#attention-mask)
            text_position_embedding (`mindspore.Tensor` of shape `(batch_size, text_seq_len)`):
                Position embeddings that are added to the queries and keys in each self-attention layer.
            text_self_attention_masks (`mindspore.Tensor` of shape `(batch_size, text_seq_len, text_seq_len)`):
            ...
            text_position_ids (`mindspore.Tensor` of shape `(batch_size, num_queries)`):
...
class MMGroundingDinoDecoder(MMGroundingDinoPreTrainedModel):
    ...
    def construct(
        ...
        Args:
            ...
            vision_encoder_hidden_states (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Last hidden state from encoder related to vision feature map.
            vision_encoder_attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask to avoid performing attention on padding pixel features. Mask values selected in `[0, 1]`:
                - 1 for pixel features that are real (i.e. **not masked**),
                - 0 for pixel features that are padding (i.e. **masked**).
            text_encoder_hidden_states (`mindspore.Tensor` of shape `(batch_size, text_seq_len, hidden_size)`):
                Last hidden state from encoder related to text features.
            text_encoder_attention_mask (`mindspore.Tensor` of shape `(batch_size, text_seq_len)`, *optional*):
                Mask to avoid performing attention on padding text features. Mask values selected in `[0, 1]`:
                - 0 for text features that are real (i.e. **not masked**),
                - 1 for text features that are padding (i.e. **masked**).
            ...
            valid_ratios (`mindspore.Tensor` of shape `(batch_size, num_feature_levels, 2)`, *optional*):
                Ratio of valid area in each feature level.
            self_attn_mask (`mindspore.Tensor` of shape `(batch_size, text_seq_len)`):
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                num_coordinates = reference_points.shape[-1]
                if num_coordinates == 4:
                    new_reference_points = tmp + mint.logit(reference_points, eps=1e-5)
                    new_reference_points = new_reference_points.sigmoid()
                elif num_coordinates == 2:
                    new_reference_points = tmp
                    new_reference_points[..., :2] = tmp[..., :2] + mint.logit(reference_points, eps=1e-5)
                    new_reference_points = new_reference_points.sigmoid()
                else:
                    raise ValueError(
...
class MMGroundingDinoModelOutput(ModelOutput):
    ...
    intermediate_reference_points (`mindspore.Tensor` of shape `(batch_size, config.decoder_layers, num_queries, 4)`):
        Stacked intermediate reference points (reference points of each layer of the decoder).
    encoder_last_hidden_state_vision (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        Sequence of hidden-states at the output of the last layer of the encoder of the model.
    encoder_last_hidden_state_text (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        Sequence of hidden-states at the output of the last layer of the encoder of the model.
    encoder_vision_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
        Tuple of `mindspore.Tensor` (one for the output of the vision embeddings + one for the output of each
        layer) of shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the vision encoder at the
        output of each layer plus the initial embedding outputs.
    encoder_text_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
        Tuple of `mindspore.Tensor` (one for the output of the text embeddings + one for the output of each layer)
        of shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the text encoder at the output of
        each layer plus the initial embedding outputs.
    encoder_attentions (`tuple(tuple(mindspore.Tensor))`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
        ...
        bi-attention heads.
    enc_outputs_class (`mindspore.Tensor` of shape `(batch_size, sequence_length, config.num_labels)`, *optional*, returned when `config.two_stage=True`):
        ...
        background).
    enc_outputs_coord_logits (`mindspore.Tensor` of shape `(batch_size, sequence_length, 4)`, *optional*, returned when `config.two_stage=True`):
        Logits of predicted bounding boxes coordinates in the first stage.
    encoder_logits (`mindspore.Tensor` of shape `(batch_size, sequence_length, config.num_labels)`, *optional*, returned when `config.two_stage=True`):
        Logits of top `config.num_queries` scoring bounding boxes in the first stage.
    encoder_pred_boxes (`mindspore.Tensor` of shape `(batch_size, sequence_length, 4)`, *optional*, returned when `config.two_stage=True`):
...
def generate_masks_with_special_tokens_and_transfer_map(input_ids: ms.Tensor) -> tuple[Tensor, Tensor]:
    ...
    Returns:
        `tuple(mindspore.Tensor)` comprising attention mask between each special tokens and position_ids:
        - **attention_mask** (`mindspore.Tensor` of shape `(batch_size, sequence_length, sequence_length)`)
    ...
    special_tokens_mask = mint.zeros((batch_size, num_token), ).to(ms.bool_)
    ...
    attention_mask = mint.eye(num_token, ).to(ms.bool_).unsqueeze(0).repeat(batch_size, 1, 1)
...
class MMGroundingDinoModel(MMGroundingDinoPreTrainedModel):
    ...
    def generate_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        Args:
            enc_output (`mindspore.Tensor[batch_size, sequence_length, hidden_size]`): Output of the encoder.
            padding_mask (`mindspore.Tensor[batch_size, sequence_length]`): Padding mask for `enc_output`.
            spatial_shapes (`mindspore.Tensor[num_feature_levels, 2]`): Spatial shapes of the feature maps.
    ...
    def construct(
        ...
        r"""
        input_ids (`mindspore.Tensor` of shape `(batch_size, text_sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.

            Indices can be obtained using [`AutoTokenizer`]. See [`BertTokenizer.__call__`] for details.
        token_type_ids (`mindspore.Tensor` of shape `(batch_size, text_sequence_length)`, *optional*):
        ...
        text_token_mask = attention_mask.to(ms.bool_)  # just to avoid renaming everywhere
        ...
        flat_areas = mint.prod(spatial_shapes, 1)
        cumsum = mint.cumsum(flat_areas, 0)
        level_start_index = mint.cat((mint.zeros((1,), dtype=spatial_shapes.dtype), cumsum[:-1]))
...
def build_label_maps(logits: ms.Tensor, input_ids: ms.Tensor) -> tuple[ms.Tensor]:
    ...
    Args:
        logits (`mindspore.Tensor` of shape `(batch_size, seq_length, hidden_size)`):
            The output logits from the model, where `hidden_size` corresponds to the dimension of the model's output features.

        input_ids (`mindspore.Tensor` of shape `(batch_size, seq_length)`):
    ...
    Returns:
        ...
        - label_maps (tuple of `mindspore.Tensor`):
    ...
    delimiter_tokens = ms.Tensor(SPECIAL_TOKENS + [0], dtype=ms.int64)
    delimiter_token_masks = (input_ids[..., None] == delimiter_tokens[None, None, :]).any(-1)
