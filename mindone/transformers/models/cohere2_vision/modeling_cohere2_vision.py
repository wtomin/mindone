...
class Cohere2VisionMultiModalProjector(ms.nn.Cell):
...
class Cohere2VisionModel(Cohere2VisionPreTrainedModel):
    ...
    def get_image_features(
        ...
        image_num_patches: ms.Tensor,
    ):
        """
        Obtains image last hidden states from the vision tower and apply multimodal projection.

        Args:
            pixel_values (`mindspore.Tensor]` of shape `(batch_size, num_patches, channels, height, width)`)
               The tensors corresponding to the input images.
            image_num_patches (`mindspore.Tensor` of shape `(num_images)`)
                Number of patches for each image.
        Returns:
            image_features (List[`mindspore.Tensor`]): List of image feature tensor, each contains all the visual feature of all patches
    ...
    def construct(
        ...
        image_num_patches: Optional[ms.Tensor] = None,
