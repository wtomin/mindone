...
from transformers.utils import add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_construct, logging
...
class IBertSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in IBertModel construct() function)
...
class IBertLayer(nn.Cell):
    ...
    def construct(
        ...
        layer_output, layer_output_scaling_factor = self.feed_construct_chunk(
    ...
    def feed_construct_chunk(self, attention_output, attention_output_scaling_factor):
...
class IBertModel(IBertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_construct(IBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
...
class IBertForMultipleChoice(IBertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_construct(IBERT_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
...
def create_position_ids_from_input_ids(input_ids, padding_idx, past_key_values_length=0):
    ...
    incremental_indices = (mint.cumsum(mask, dim=1).to(mask.dtype) + past_key_values_length) * mask
