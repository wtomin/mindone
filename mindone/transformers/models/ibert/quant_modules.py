...
class Function:
    """Base class for custom functions."""
    @staticmethod
...
class QuantEmbedding(nn.Cell):
    """
    Quantized version of `mint.mint.nn.Embedding`. Adds quantization-specific arguments on top of `mint.mint.nn.Embedding`.
...
class QuantLinear(nn.Cell):
    """
    Quantized version of `mint.mint.nn.Linear`. Adds quantization-specific arguments on top of `mint.mint.nn.Linear`.
...
class IntGELU(nn.Cell):
    """
    Quantized version of `mint.mint.nn.GELU`. Adds quantization-specific arguments on top of `mint.mint.nn.GELU`.
...
class IntSoftmax(nn.Cell):
    """
    Quantized version of `mint.mint.nn.Softmax`. Adds quantization-specific arguments on top of `mint.mint.nn.Softmax`.
...
class IntLayerNorm(nn.Cell):
    """
    Quantized version of `mint.mint.nn.LayerNorm`. Adds quantization-specific arguments on top of `mint.mint.nn.LayerNorm`.
    ...
    def construct(self, x, scaling_factor=None):
        ...
        if self.dim_sqrt is None:
            ...
            self.dim_sqrt = mint.sqrt(n).to(x.device)
...
def get_percentile_min_max(input, lower_percentile, upper_percentile, output_tensor=False):
    ...
    Returns:
        `Tuple(ms.Tensor, ms.Tensor)`: Percentile min and max value of *input*
    """
    ...
    upper_bound = mint.kthvalue(input, k=upper_index)[0]

    if lower_percentile == 0:
        lower_bound = upper_bound * 0
        # lower_index += 1
    else:
        lower_bound = -mint.kthvalue(-input, k=lower_index)[0]
...
def linear_quantize(input, scale, zero_point, inplace=False):
    ...
    Returns:
        `ms.Tensor`: Linearly quantized value of *input* according to *scale* and *zero_point*.
    """
...
def symmetric_linear_quantization_params(num_bits, saturation_min, saturation_max, per_channel=False):
    ...
    Args:
        saturation_min (`ms.Tensor`):
            Lower bound for quantization range.
        saturation_max (`ms.Tensor`):
            Upper bound for quantization range.
        per_channel (`bool`, *optional*, defaults to `False`):
            Whether to or not use channel-wise quantization.

    Returns:
        `ms.Tensor`: Scaling factor that linearly quantizes the given range between *saturation_min* and
        *saturation_max*.
    """
...
class SymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, percentile_mode, scale):
        ...
        Returns:
            `ms.Tensor`: Symmetric-quantized value of *input*.
        """
...
class floor_ste(Function):
    """
    Straight-through Estimator(STE) for mint.floor()
    """
...
class round_ste(Function):
    """
    Straight-through Estimator(STE) for mint.round()
    """
...
def batch_frexp(inputs, max_bit=31):
    ...
    Args:
        scaling_factor (`ms.Tensor`):
            Target scaling factor to decompose.

    Returns:
        ``Tuple(ms.Tensor, ms.Tensor)`: mantisa and exponent
    """
...
class FixedPointMul(Function):
    ...
    Args:
        pre_act (`ms.Tensor`):
            Input tensor.
        pre_act_scaling_factor (`ms.Tensor`):
            Scaling factor of the input tensor *pre_act*.
        bit_num (`int`):
            Quantization bitwidth.
        z_scaling_factor (`ms.Tensor`):
            Scaling factor of the output tensor.
        identity (`ms.Tensor`, *optional*):
            Identity tensor, if exists.
        identity_scaling_factor (`ms.Tensor`, *optional*):
            Scaling factor of the identity tensor *identity*, if exists.

    Returns:
        `ms.Tensor`: Output tensor(*pre_act* if *identity* is not given, otherwise the addition of *pre_act* and
    ...
    def forward(
        ...
        with _no_grad():
            ...
            _A = pre_act_scaling_factor.to(ms.float64)
            _B = (z_scaling_factor.to(ms.float32)).to(ms.float64)
            new_scale = _A / _B
            new_scale = reshape(new_scale)

            m, e = batch_frexp(new_scale)

            output = z_int.to(ms.float64) * m.to(ms.float64)
            ...
            if identity is not None:
                ...
                _A = identity_scaling_factor.to(ms.float64)
                _B = (z_scaling_factor.to(ms.float32)).to(ms.float64)
                new_scale = _A / _B
                new_scale = reshape(new_scale)

                m1, e1 = batch_frexp(new_scale)
                output1 = wx_int.to(ms.float64) * m1.to(ms.float64)
                output1 = mint.round(output1 / (2.0**e1))

                output = output1 + output

            return mint.clamp(output.to(ms.float32), -n - 1, n)
