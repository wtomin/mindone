...
def or_masks(*mask_functions: list[Callable]) -> Callable:
    ...
    def or_mask(batch_idx, head_idx, q_idx, kv_idx):
        result = mint.zeros((), dtype=ms.bool_)
        for mask in mask_functions:
            result = result | mask(batch_idx, head_idx, q_idx, kv_idx)
        return result
...
def padding_mask_function(padding_mask: Tensor) -> Callable:
    def inner_mask(batch_idx: int, head_idx: int, q_idx: int, kv_idx: int) -> bool:
...
def prepare_padding_mask(
    attention_mask: Optional[Tensor], kv_length: int, kv_offset: int, _slice: bool = True
) -> Optional[Tensor]:
    """
...
def _ignore_causal_mask_sdpa(
    ...
    if (
        ...
        and (padding_mask is None or mint.all(padding_mask))
    ):
        return True
...
def sdpa_mask_recent_mindspore(
    batch_size: int,
...
sdpa_mask = sdpa_mask_recent_mindspore
...
def eager_mask(
    ...
    **kwargs,
    ...
    mask = mint.where(mask, Tensor(0.0, dtype=dtype), min_dtype)
    return mask
...
def _preprocess_mask_arguments(
    ...
    input_embeds: Tensor,
    attention_mask: Optional[Union[Tensor, BlockMask]],
    cache_position: Tensor,
    past_key_values: Optional[Cache],
    layer_idx: Optional[int],
) -> tuple[bool, Optional[Union[Tensor, BlockMask]], int, int]:
    """
    ...
    if isinstance(attention_mask, (Tensor, BlockMask)) and len(attention_mask.shape) == 4:
...
) -> Optional[Union[Tensor, BlockMask]]:
    """
...
def tensor_to_mask_visual(original_tensor: Tensor, grid_size=(20, 40), style="majong") -> str:
...
class AttentionMask(Tensor):
    """MindSpore version of AttentionMask"""

    def __new__(cls, data, style=None):
        cls.style = style
        return Tensor._make_subclass(cls, data, require_grad=False)
    ...
    def to_string(self, grid_size=(20, 40), limit=4):
        ...
        *batch_dims, num_rows, num_cols = dense_mask.shape
        ...
        total_vis.append(f"Tensor(shape={tuple(self.shape)}, dtype={self.dtype})")
        return "\n".join(total_vis)
    ...
    @classmethod
    def from_tensor(cls, tensor: Tensor, style: Optional[str] = None) -> "AttentionMask":
        res = cls(tensor)
        res.style = style
        return res
