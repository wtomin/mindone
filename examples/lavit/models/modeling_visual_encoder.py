...
class SwiGLU(nn.Cell):
    def __init__(
        ...
        self.w1 = nn.Dense(in_features, hidden_features)
        self.w2 = nn.Dense(in_features, hidden_features)

        self.act = act_layer()
        self.ffn_ln = norm_layer(hidden_features) if subln else nn.Identity()
        self.w3 = nn.Dense(hidden_features, out_features)
...
class Attention(nn.Cell):
    def __init__(
        ...
        self.subln = subln
        if self.subln:
            self.q_proj = nn.Dense(dim, all_head_dim, has_bias=False)
            self.k_proj = nn.Dense(dim, all_head_dim, has_bias=False)
            self.v_proj = nn.Dense(dim, all_head_dim, has_bias=False)
        else:
        ...
        if window_size:
            ...
            coords = ops.stack(ms.numpy.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww
        ...
        # self.proj = nn.Dense(all_head_dim, all_head_dim)
    ...
    def construct(self, x, rel_pos_bias=None, attn_mask=None):
        B, N, C = x.shape
        if self.subln:
            q = ops.dense(input=x, weight=self.q_proj.weight, bias=self.q_bias)
            k = ops.dense(input=x, weight=self.k_proj.weight, bias=None)
            v = ops.dense(input=x, weight=self.v_proj.weight, bias=self.v_bias)
        ...
        if self.rope:
            ...
            q = ops.cat((q[:, :, :1, :], ro_q_t), -2).to(v.dtype)

            k_t = k[:, :, 1:, :]
            ro_k_t = self.rope(k_t)
            k = ops.cat((k[:, :, :1, :], ro_k_t), -2).to(v.dtype)
        ...
        if self.xattn:
            ...
            if rel_pos_bias is not None:
                attn = attn + rel_pos_bias.to(attn.dtype)
...
class Block(nn.Cell):
    def __init__(
        ...
        if init_values is not None and init_values > 0:
            self.gamma_1 = ms.Parameter(init_values * ops.zeros((dim)), requires_grad=True)
            self.gamma_2 = ms.Parameter(init_values * ops.zeros((dim)), requires_grad=True)
        else:
...
class RelativePositionBias(nn.Cell):
    def __init__(self, window_size, num_heads):
        ...
        relative_position_index = ms.Parameter(relative_position_index, requires_grad=False)
...
class EVAVisionTransformer(nn.Cell):
    ...
    def __init__(
        ...
        dpr = [x.item() for x in ms.numpy.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
    ...
    def construct_features(self, x, return_all_features=False):
    ...
    def construct(self, x, return_all_features=False):
        if return_all_features:
            return self.construct_features(x, return_all_features)
...
class CLIPVisionCfg:
    ...
    patch_dropout: float = 0.0  # what fraction of patches to dropout during training
