...
def convert_meta_torch_ckpt(ckpt_dir, output_name, dtype=ms.float16):
    ...
    try:
        ...
        raise ImportError("Failed to load pytorch checkpoint. Please make sure pytorch is available.")
    dic = {
    ...
    for name in checkpoints[0].keys():
        for k, v in dic.items():
            if k in name:
                if v is not None:
                    value = np.concatenate([checkpoints[i][name].numpy() for i in range(len(checkpoints))], v)
                else:
...
def convert_pt_to_ms(input_path, output_path, dtype=None, **kwargs):
    ...
    try:
        ...
        raise ImportError("Failed to load huggingface checkpoint. Please make sure transformers is available.")
    ...
    print(
        f"\rConvert huggingface checkpoint finished, the mindspore checkpoint is saved in '{output_path}'.", flush=True
    )
    return True
...
def convert_qkv_concat_weight(param_dict):
    ...
    for i in range(assume_num_layers):
        ...
        param_dict[gate_hidden_concat_weight_name] = ms.Parameter(
