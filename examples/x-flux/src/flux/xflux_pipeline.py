...
class XFluxPipeline:
    ...
    def set_ip(self, local_path: str = None, repo_id = None, name: str = None):
        ...
        self.improj = self.improj.to(dtype=mindspore.bfloat16)
        ...
        for name, _ in self.model.attn_processors.items():
            ...
            if ip_state_dict:
                ...
                ip_attn_procs[name].to(dtype=mindspore.bfloat16)
            else:
    ...
    def __call__(self,
        ...
        if self.controlnet_loaded:
            ...
            controlnet_image = mindspore.Tensor.from_numpy((np.array(controlnet_image) / 127.5) - 1)
    ...
    def construct(
        ...
        x = get_noise(
            1, height, width, dtype=mindspore.bfloat16, seed=seed
        )
