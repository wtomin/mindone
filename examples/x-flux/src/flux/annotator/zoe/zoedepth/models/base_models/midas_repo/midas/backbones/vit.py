...
def _make_vit_b_rn50_backbone(
    ...
    for s in range(used_number_stages):
        value = mindspore.nn.SequentialCell(mindspore.mint.nn.Identity(), mindspore.mint.nn.Identity(), mindspore.mint.nn.Identity())
        exec(f"pretrained.act_postprocess{s + 1}=value")
    for s in range(used_number_stages, 4):
        if s < number_stages:
            final_layer = mindspore.mint.nn.ConvTranspose2d(
            ...
            final_layer = mindspore.mint.nn.Conv2d(
        ...
        value = mindspore.nn.SequentialCell(*layers)
        exec(f"pretrained.act_postprocess{s + 1}=value")
