...
class DPTDepthModel(DPT):
    def __init__(self, path=None, non_negative=True, **kwargs):
        ...
        head = mindspore.nn.SequentialCell(
            mindspore.mint.nn.Conv2d(head_features_1, head_features_1 // 2, kernel_size=3, stride=1, padding=1),
            Interpolate(scale_factor=2, mode="bilinear", align_corners=True),
            mindspore.mint.nn.Conv2d(head_features_1 // 2, head_features_2, kernel_size=3, stride=1, padding=1),
            mindspore.mint.nn.ReLU(True),
            mindspore.mint.nn.Conv2d(head_features_2, 1, kernel_size=1, stride=1, padding=0),
    ...
    def construct(self, x):
        return super().construct(x).squeeze(dim=1)
