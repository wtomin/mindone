...
from mindspore.utils.checkpoint import checkpoint
...
def _get_rel_pos_bias(self, window_size):
    ...
    new_sub_table = mindspore.mint.nn.functional.interpolate(old_sub_table, size=(new_height, new_width), mode="bilinear")
    new_sub_table = new_sub_table.permute(0, 2, 3, 1).reshape(new_num_relative_distance - 3, -1)

    new_relative_position_bias_table = mindspore.mint.cat(
...
def attention_forward(self, x, resolution, shared_rel_pos_bias: Optional[mindspore.Tensor] = None):
    """
    Modification of timm.models.beit.py: Attention.forward to support arbitrary window sizes.
    """
    B, N, C = x.shape

    qkv_bias = mindspore.mint.cat((self.q_bias, self.k_bias, self.v_bias)) if self.q_bias is not None else None
    qkv = mindspore.mint.nn.functional.linear(input=x, weight=self.qkv.weight, bias=qkv_bias)
...
def block_forward(self, x, resolution, shared_rel_pos_bias: Optional[mindspore.Tensor] = None):
    """
...
def beit_forward_features(self, x):
    ...
    x = mindspore.mint.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
