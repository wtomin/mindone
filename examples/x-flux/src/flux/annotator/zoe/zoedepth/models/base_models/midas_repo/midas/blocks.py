...
def _make_scratch(in_shape, out_shape, groups=1, expand=False):
    ...
    scratch.layer1_rn = mindspore.mint.nn.Conv2d(
        in_shape[0], out_shape1, kernel_size=3, stride=1, padding=1, bias=False, groups=groups
    )
    scratch.layer2_rn = mindspore.mint.nn.Conv2d(
        in_shape[1], out_shape2, kernel_size=3, stride=1, padding=1, bias=False, groups=groups
    )
    scratch.layer3_rn = mindspore.mint.nn.Conv2d(
        in_shape[2], out_shape3, kernel_size=3, stride=1, padding=1, bias=False, groups=groups
    )
    if len(in_shape) >= 4:
        scratch.layer4_rn = mindspore.mint.nn.Conv2d(
...
def _make_pretrained_efficientnet_lite3(use_pretrained, exportable=False):
    try:
        ...
        print("Can't import torch, please install it with: pip install torch")
...
def _make_efficientnet_backbone(effnet):
    pretrained = mindspore.nn.Cell()

    pretrained.layer1 = mindspore.nn.SequentialCell(
        effnet.conv_stem, effnet.bn1, effnet.act1, *effnet.blocks[0:2]
    )
    pretrained.layer2 = mindspore.nn.SequentialCell(*effnet.blocks[2:3])
    pretrained.layer3 = mindspore.nn.SequentialCell(*effnet.blocks[3:5])
    pretrained.layer4 = mindspore.nn.SequentialCell(*effnet.blocks[5:9])
...
class Interpolate(mindspore.nn.Cell):
    ...
    def __init__(self, scale_factor, mode, align_corners=False):
        ...
        self.interp = mindspore.mint.nn.functional.interpolate
...
class ResidualConvUnit(mindspore.nn.Cell):
    ...
    def __init__(self, features):
        ...
        self.conv1 = mindspore.mint.nn.Conv2d(
            features, features, kernel_size=3, stride=1, padding=1, bias=True
        )

        self.conv2 = mindspore.mint.nn.Conv2d(
            features, features, kernel_size=3, stride=1, padding=1, bias=True
        )

        self.relu = mindspore.mint.nn.ReLU(inplace=True)
...
class FeatureFusionBlock(mindspore.nn.Cell):
    ...
    def construct(self, *xs):
        ...
        output = mindspore.mint.nn.functional.interpolate(
...
class ResidualConvUnit_custom(mindspore.nn.Cell):
    ...
    def __init__(self, features, activation, bn):
        ...
        if self.bn==True:
            self.bn1 = mindspore.mint.nn.BatchNorm2d(features)
            self.bn2 = mindspore.mint.nn.BatchNorm2d(features)
...
class FeatureFusionBlock_custom(mindspore.nn.Cell):
    ...
    def __init__(self, features, activation, deconv=False, bn=False, expand=False, align_corners=True, size=None):
        ...
        self.out_conv = mindspore.mint.nn.Conv2d(features, out_features, kernel_size=1, stride=1, padding=0, bias=True, groups=1)
    ...
    def construct(self, *xs, size=None):
        """Forward pass.
