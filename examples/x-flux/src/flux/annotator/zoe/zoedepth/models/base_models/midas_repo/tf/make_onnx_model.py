...
import cv2
import numpy as np
from mindsporevision.transforms import Compose, Normalize
...
class MidasNet_preprocessing(MidasNet):
    ...
    def construct(self, x):
        ...
        mean = mindspore.Tensor([0.485, 0.456, 0.406])
        std = mindspore.Tensor([0.229, 0.224, 0.225])
...
def run(model_path):
    ...
    with mindspore._no_grad():
        sample = mindspore.Tensor.from_numpy(img_input).unsqueeze(0)
