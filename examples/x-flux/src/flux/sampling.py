...
def get_schedule(
    ...
    # extra step for zero
    timesteps = mindspore.mint.linspace(1, 0, num_steps + 1)
...
def denoise(
    ...
    guidance_vec = mindspore.ops.full((img.shape[0],), guidance, dtype=img.dtype)
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = mindspore.ops.full((img.shape[0],), t_curr, dtype=img.dtype, )
        pred = model(
            img=img,
            img_ids=img_ids,
            txt=txt,
