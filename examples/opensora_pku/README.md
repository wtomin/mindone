...
+ --val_interval 1 \
```
The edits allow to compute the loss on the validation set specified by `merge_data_val.txt` for every 1 epoch (defined by `val_interval`). The validation loss will be recorded in the `result_val.log` under the output directory.
