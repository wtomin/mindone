...
num_frames=93  # to sample m frames from a single video. The total number of images： num_frames + use_image_num
...
output_dir=t2v-f$num_frames-$height-$width-img$use_image_num-videovae488-$model_dtype-FA$enable_flash_attention-bs$batch_size-t5_sp
...
msrun --bind_core=True --worker_num=8 --local_worker_num=8 --master_port=9000 --log_dir=$output_dir/parallel_logs opensora/train/train_t2v.py \
    ...
    --video_data "scripts/train_data/single_video_data_513.txt" \
