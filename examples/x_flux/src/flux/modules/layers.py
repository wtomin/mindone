...
class LoRALinearLayer(nn.Cell):
    def __init__(self, in_features, out_features, rank=4, network_alpha=None, dtype=None):
        super().__init__()
...
class LoraFluxAttnProcessor(nn.Cell):
...
class DoubleStreamBlockLoraProcessor(nn.Cell):
    def __init__(self, dim: int, rank=4, network_alpha=None, lora_weight=1):
        super().__init__()
...
class IPDoubleStreamBlockProcessor(nn.Cell):
    ...
    def __init__(self, context_dim, hidden_dim):
        ...
        zeros_(self.ip_adapter_double_stream_k_proj.weight)
        zeros_(self.ip_adapter_double_stream_k_proj.bias)

        zeros_(self.ip_adapter_double_stream_v_proj.weight)
        zeros_(self.ip_adapter_double_stream_v_proj.bias)
...
class IPSingleStreamBlockProcessor(nn.Cell):
    ...
    def __init__(self, context_dim, hidden_dim):
        ...
        zeros_(self.ip_adapter_single_stream_k_proj.weight)
        zeros_(self.ip_adapter_single_stream_v_proj.weight)
...
class SingleStreamBlockLoraProcessor(nn.Cell):
    ...
    def construct(self, attn: nn.Cell, x: Tensor, vec: Tensor, pe: Tensor) -> Tensor:
        mod, _ = attn.modulation(vec)
        x_mod = (1 + mod.scale) * attn.pre_norm(x) + mod.shift
        qkv, mlp = mint.split(attn.linear1(x_mod), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1)
...
class SingleStreamBlock(nn.Cell):
    ...
    def construct(
        self, x: Tensor, vec: Tensor, pe: Tensor, image_proj: Tensor | None = None, ip_scale: float = 1.0
    ) -> Tensor:
        if image_proj is None:
